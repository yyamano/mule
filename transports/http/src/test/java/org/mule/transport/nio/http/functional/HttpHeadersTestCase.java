...
public class HttpHeadersTestCase extends AbstractServiceAndFlowTestCase
{
    ...
    public HttpHeadersTestCase(final ConfigVariant variant, final String configResources)
    {
    ...
    {
        return Arrays.asList(new Object[][]{{ConfigVariant.SERVICE, "http-headers-config-service.xml"},
    ...
    public void testJettyHeaders() throws Exception
    {
        ...
        final MuleMessage result = client.send("clientEndpoint", null, null);

        final String contentTypeProperty = result.getInboundProperty(HttpConstants.HEADER_CONTENT_TYPE);
        assertNotNull(contentTypeProperty);
        assertEquals("application/x-download", contentTypeProperty);

        final String contentDispositionProperty = result.getInboundProperty(HttpConstants.HEADER_CONTENT_DISPOSITION);
