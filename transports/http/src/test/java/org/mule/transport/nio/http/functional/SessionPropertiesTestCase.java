...
public class SessionPropertiesTestCase extends FunctionalTestCase
{
    ...
    public void testHttp1ToHttp2ToHttp3SessionPropertiesTestCase() throws Exception
    {
        ...
        final MuleMessage response = client.send(HttpConnector.HTTP + "://localhost:" + dynamicPort1.getNumber() + "/Flow1s1",
            "some message", Collections.emptyMap());
    ...
    {
        final MuleClient client = new MuleClient(muleContext);
        final MuleMessage response = client.send(HttpConnector.HTTP + "://localhost:" + dynamicPort4.getNumber() + "/Flow1s2",
            "some message", Collections.emptyMap());
        assertNotNullAndNotExceptionResponse(response);
    }

    private void assertNotNullAndNotExceptionResponse(final MuleMessage response)
    {
