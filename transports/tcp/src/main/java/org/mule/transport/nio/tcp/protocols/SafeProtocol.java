...
/**
 * This precedes every message with a cookie. It should probably not be used in
 * production. We use ths protocol as the default because previously people tended to
 * use DefaultProtocol without considering packet fragmentation etc. You should
 * probably change to LengthProtocol. Remember - both sender and receiver must use
 * the same protocol.
 */
public class SafeProtocol implements TcpProtocol
{
    ...
    private boolean assertSiblingSafe(InputStream is) throws IOException
    {
        ...
        if (null != cookie)
        {
            if (!(cookie instanceof byte[] && ((byte[]) cookie).length == COOKIE.length() && COOKIE.equals(new String(
                (byte[]) cookie))))
            {
                helpUser();
            }
    ...
    private void helpUser(Exception e) throws IOException
    {
        throw (IOException) new IOException(
            "An error occurred while verifying your connection.  "
