 ...
 * {@link org.mule.transport.tcp.protocols.XmlMessageEOFProtocol} may be more
 * reliable.
 * </p>
 * <p>
...
public class XmlMessageProtocol extends AbstractByteProtocol
{
    ...
    private final ConcurrentMap<InputStream, PushbackInputStream> pbMap = new ConcurrentHashMap<InputStream, PushbackInputStream>();
    ...
    {
        PushbackInputStream pbis = pbMap.get(is);
        if (null == pbis)
        {
            pbis = new PushbackInputStream(is, PUSHBACK_BUFFER_SIZE);
            final PushbackInputStream prev = pbMap.putIfAbsent(is, pbis);
        ...
        try
        {
            ...
            final StringBuffer message = new StringBuffer(READ_BUFFER_SIZE);
            int patternIndex = -1;
            boolean repeat;
            do
            {
                ...
                if (len >= 0)
                {
                    ...
                    final int available = safeAvailable(pbis);
                    repeat = isRepeat(patternIndex, len, available);
                    if (logger.isDebugEnabled())
                    {
                        logger.debug(String.format(
                            "Pattern Index: %d + Len: %d + Available: %d -> Repeat: %s", patternIndex, len,
                            available, repeat));
                    }
                }
                else
    ...
    /**
     * Show we continue reading?
     * 
     * @see XmlMessageEOFProtocol
     * @param patternIndex The index of the xml tag (or -1 if the next message not
     *            found)
     ...
     */
    protected boolean isRepeat(final int patternIndex, final int len, final int available)
    {
