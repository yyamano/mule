 ...
 * The socket is read until no more bytes are (momentarily) available (previously the
 * transfer buffer also had to be full on the previous read, which made stronger
 * requirements on the underlying network). On slow networks
...
public class DirectProtocol extends AbstractByteProtocol
{
    ...
    public DirectProtocol(final boolean streamOk, final int bufferSize)
    {
        super(streamOk);
    ...
    public Object read(final InputStream is, final int limit) throws IOException
    {
        ...
        try
        {
            ...
            do
            {
                ...
                final boolean isRepeat = isRepeat(len, available);
                repeat = (len != EOF) && (remain > 0) && isRepeat;

                if (logger.isDebugEnabled())
                {
                    logger.debug(MessageFormat.format(
                        "len/limit/remain/available/isRepeat()/repeat: {0}/{1}/{2}/{3}/{4}/{5} for: {6}",
                        len, limit, remain, available, isRepeat, repeat, is));
                }
            }
            while (repeat);
        }
    ...
    protected int remaining(final int limit, final int remain, final int len)
    {
        if (UNLIMITED == limit)
        {
            return bufferSize;
    ...
    /**
     * Decide whether to repeat transfer. This implementation does so if more data
     * are available. Note that previously, while documented as such, there was also
     * the additional requirement that the previous transfer completely used the
     * transfer buffer.
     * 
