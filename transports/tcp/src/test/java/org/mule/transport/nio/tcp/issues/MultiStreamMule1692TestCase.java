...
public class MultiStreamMule1692TestCase extends AbstractServiceAndFlowTestCase
{
    ...
    public MultiStreamMule1692TestCase(final ConfigVariant variant, final String configResources)
    {
    ...
    {
        return Arrays.asList(new Object[][]{{ConfigVariant.SERVICE, "nio/tcp-streaming-test-service.xml"},
            {ConfigVariant.FLOW, "nio/tcp-streaming-test-flow.xml"}});
    }
    ...
    public void testSend() throws Exception
    {
        ...
        ((FunctionalStreamingTestComponent) ftc).setEventCallback(newCallback(latch, message),
            TEST_MESSAGE.length());
        client.dispatch(
        ...
        ((FunctionalStreamingTestComponent) ftc).setEventCallback(newCallback(latch2, message2),
            TEST_MESSAGE_2.length());
        client.dispatch(
            ((InboundEndpoint) client.getMuleContext().getRegistry().lookupObject("testInbound")).getAddress(),
